...
class Codebase
{
    ...
    /**
     * @param array<string,string> $queue (Precondition: Not empty)
     * @return string
     */
    private static function popString(array &$queue) {
        /** @var string $value */
        $value = reset($queue);
        /** @var string $key */
        $key = key($queue);
    ...
    public function scanFiles()
    {
        ...
        while ($this->files_to_scan || $this->classes_to_scan) {
            if ($this->files_to_scan) {
                // array_shift is slow, rekeys entire array, use SqlQueue or reset(), unset($key) instead
                $file_path = self::popString($this->files_to_scan);
                ...
                // array_shift is slow, rekeys entire array, use SqlQueue or reset(), unset($key) instead
                $fq_classlike_name = self::popString($this->classes_to_scan);
    ...
    public function analyzeFiles(ProjectChecker $project_checker, $pool_size, $alter_code)
    {
        ...
        // but before analysis of methods, functions, etc, and before forking and after getting a server request.
